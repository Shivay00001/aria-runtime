...
class AgentKernel:
    ...
    def run(self, request: SessionRequest) -> SessionResult:
        ...
        self._memory.append_message(
            session_id, Message(role=MessageRole.USER, content=request.task)
        )
        ...
        ctx = ExecutionContext(
            ...
            available_tools=self._registry.all_manifests,
            config=self._config,
        )
        ...
        try:
            ...
            _log.critical(
                "unexpected_error",
                extra={"session_id": session_id, "error_type": error_type, "trace": full_trace},
            )
            self._emit(
                session_id,
                None,
        ...
        self._sync_session(
            session_id, fsm, step_count, total_cost, error_type=error_type, error_msg=error_msg
        )
    ...
    def _execute_tool(
        self, session_id: str, step_id: str, manifest: object, arguments: dict, tool_call_id: str
    ) -> ToolResult:
        ...
        try:
            result = run_tool_sandboxed(
                manifest=manifest, arguments=arguments, tool_module_path=module_path
            )
        except (
            ToolTimeoutError,
            ToolSandboxError,
    ...
    def _emit(
        self, session_id: str, step_id: str | None, event_type: str, level: LogLevel, payload: dict
    ) -> None:
        try:
            ...
            _log.critical(
                "audit_emit_failed", extra={"event_type": event_type, "session_id": session_id}
            )
            raise
