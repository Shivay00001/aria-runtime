...
class ModelRouter:
    def __init__(self, providers: dict[str, ModelProviderInterface], audit_writer=None) -> None:
        if not providers:
    ...
    def call(self, request: PromptRequest) -> RawModelResponse:
        ...
        if provider is None:
            raise ValueError(
                f"Provider {request.provider!r} not registered. Available: {list(self._providers)}"
            )
        ...
        for attempt in range(1, _MAX_RETRIES + 1):
            try:
                ...
                _log.warning(
                    ...
                    extra={"provider": request.provider, "session_id": request.session_id},
                )
                raise
            ...
            try:
                ...
                if attempt < _MAX_RETRIES:
                    delay = min(
                        _BASE_BACKOFF * (2 ** (attempt - 1)) + random.uniform(0, 1), _MAX_BACKOFF
                    )
                    time.sleep(delay)

        raise ModelProviderExhaustedError(
            f"Provider {request.provider!r} failed after {_MAX_RETRIES} attempts. Last: {last_err}",
