...
@click.pass_context
def run_command(ctx: click.Context, task: str, provider: str | None,
                model: str | None, max_steps: int | None, dry_run: bool) -> None:
    ...
    try:
        ...
        click.echo(f"Config error: {e}", err=True); sys.exit(1)
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        click.echo(f"Startup error: {e}", err=True); sys.exit(1)

    request = SessionRequest(task=task, provider_override=provider,
                              model_override=model, max_steps_override=max_steps)
    ...
    try:
        ...
        click.echo(f"Critical error: {e}", err=True); sys.exit(1)
    finally:
        storage.close()
