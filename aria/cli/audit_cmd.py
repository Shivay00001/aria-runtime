...
def audit_list(ctx: click.Context, last: int) -> None:
    ...
    if not sessions:
        click.echo("No sessions."); return
    click.echo(f"{'SESSION ID':<38} {'STATUS':<12} {'STEPS':<6} {'COST':>8}  {'TASK'}")
    click.echo("-" * 90)
    for s in sessions:
        click.echo(f"{s['session_id']:<38} {s['status']:<12} {s.get('total_steps',0):<6} "
...
def audit_export(ctx: click.Context, session_id: str, fmt: str) -> None:
    ...
    try:
        ...
        click.echo(f"Error: {e}", err=True); sys.exit(1)
    if not events:
        click.echo(f"No events for session {session_id!r}."); return
    if fmt == "json":
        ...
        for e in events:
            ...
            click.echo(f"[{e.get('timestamp','')[:19]}] {e.get('level',''):<8} "
...
def audit_verify(ctx: click.Context, session_id: str) -> None:
    ...
    if ok:
        ...
        click.echo(f"[FAIL] Audit chain BROKEN for {session_id!r}", err=True); sys.exit(1)
