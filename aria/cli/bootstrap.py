...
from aria.models.types import KernelConfig
...
def load_config(config_path: str = "") -> KernelConfig:
    ...
    kw: dict = {}
    _e = os.environ.get
    if _e("ARIA_PRIMARY_PROVIDER"):
        kw["primary_provider"] = _e("ARIA_PRIMARY_PROVIDER")
    if _e("ARIA_PRIMARY_MODEL"):
        kw["primary_model"] = _e("ARIA_PRIMARY_MODEL")
    if _e("ARIA_MAX_STEPS"):
        try:
            kw["max_steps"] = int(_e("ARIA_MAX_STEPS"))  # type: ignore
        except ValueError:
            pass
    if _e("ARIA_MAX_COST_USD"):
        try:
            kw["max_cost_usd"] = float(_e("ARIA_MAX_COST_USD"))  # type: ignore
        except ValueError:
            pass
    if _e("ARIA_DB_PATH"):
        kw["db_path"] = _e("ARIA_DB_PATH")
    if _e("ARIA_LOG_PATH"):
        kw["log_path"] = _e("ARIA_LOG_PATH")
    return KernelConfig(**kw)


def build_kernel(
    config: KernelConfig, log_level: str = "INFO"
) -> tuple[AgentKernel, SQLiteStorage]:
